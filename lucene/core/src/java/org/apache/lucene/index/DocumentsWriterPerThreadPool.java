...
final class DocumentsWriterPerThreadPool implements Iterable<DocumentsWriterPerThread>, Closeable {
  ...
  synchronized int size() {
    return dwpts.get(SegmentBucket.DEFAULT).size() + dwpts.get(SegmentBucket.ONE).size();
  }
  ...
  DocumentsWriterPerThread getAndLock(SegmentBucket bucket) {
    ensureOpen();
    DocumentsWriterPerThread dwpt = freeList.get(bucket).poll(DocumentsWriterPerThread::tryLock);
    if (dwpt != null) {
      return dwpt;
    }
  ...
  private synchronized boolean contains(DocumentsWriterPerThread state) {
    return dwpts.get(state.bucket).contains(state);
  }
  ...
  void marksAsFreeAndUnlock(DocumentsWriterPerThread state) {
    ...
    freeList.get(state.bucket).add(state, ramBytesUsed);
    state.unlock();
  }
  ...
  public synchronized Iterator<DocumentsWriterPerThread> iterator() {
    ...
    list.addAll(List.copyOf(dwpts.get(SegmentBucket.DEFAULT)));
    list.addAll(List.copyOf(dwpts.get(SegmentBucket.ONE)));
  ...
  synchronized boolean checkout(DocumentsWriterPerThread perThread) {
    ...
    if (dwpts.get(perThread.bucket).remove(perThread)) {
  ...
  synchronized boolean isRegistered(DocumentsWriterPerThread perThread) {
    return dwpts.get(perThread.bucket).contains(perThread);
  }
